 ...
 */
class PresenceLightAgent : AbstractDelayedTriggerableAgent(DelayMode.DELAY_DEACTIVATION, MAX_TIMEOUT) {

    var locationRemote: LocationRemote? = null

    @Throws(InitializationException::class, InterruptedException::class)
    override fun init(config: UnitConfigType.UnitConfig) {
        super.init(config)
        try {
            locationRemote = Units.getUnit(getConfig().placementConfig.locationId, false, Units.LOCATION)
                ?.also { location ->

                    // activation trigger
                    registerActivationTrigger(
                        GenericServiceStateValueTrigger(
                            location,
    ...
    override fun getDelayScaleFactor(): Double {
        return locationRemote?.comfortToEconomyRatio ?: 1.0
    }
    ...
    companion object {
         ...
         */
        val MAX_TIMEOUT = TimeUnit.MINUTES.toMillis(30)
