...
public class CachedTemplateRegistryRemote {
     ...
     */
    public static TemplateRegistryRemote getRegistry(final boolean waitForData) throws CouldNotPerformException, InterruptedException {
        if (waitForData) {
            waitForData();
        }
        return getRegistry();
     ...
     * @throws NotAvailableException if the initial startup of the TemplateRegistryRemote fails
     */
    public static TemplateRegistryRemote getRegistry() throws NotAvailableException {
        try {
            if (shutdown) {
