        ...
        try {
            ...
            try {
                ...
                String className = PRESET_APP_PACKAGE_PREFIX
                        + "." + StringProcessor.removeWhiteSpaces(LabelProcessor.getLabelByLanguage(Locale.ENGLISH, appClass.getLabel())) + "App";
                app = (AppController) Thread.currentThread().getContextClassLoader().loadClass(className).getConstructor().newInstance();
            } catch (CouldNotPerformException | ClassNotFoundException | SecurityException | InstantiationException |
            ...
            app.init(appUnitConfig);
                 ...
                 IllegalAccessException | IllegalArgumentException | InterruptedException | NoSuchMethodException |
                 InvocationTargetException ex) {
