...
public class UnitSchemaModule extends SchemaModule {
    ...
    ImmutableList<UnitData> units(@Arg("filter") UnitFilter unitFilter) throws BCOGraphQLError {
        try {
            ...
            for (UnitConfig unitConfig : Registries.getUnitRegistry(ServerError.BCO_TIMEOUT_SHORT, ServerError.BCO_TIMEOUT_TIME_UNIT).getUnitConfigs(unitFilter)) {
    ...
    @Mutation("units")
    ImmutableList<UnitData> units(@Arg("filter") UnitFilter unitFilter, @Arg("data") UnitData data, DataFetchingEnvironment env) throws BCOGraphQLError {
        try {
            final List<UnitData> dataList = new ArrayList<>();
            for (UnitConfig unitConfig : Registries.getUnitRegistry(ServerError.BCO_TIMEOUT_SHORT, ServerError.BCO_TIMEOUT_TIME_UNIT).getUnitConfigs(unitFilter)) {
