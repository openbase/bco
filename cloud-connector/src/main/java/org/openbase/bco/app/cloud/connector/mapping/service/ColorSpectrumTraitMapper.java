    ...
    protected ColorState map(JsonObject jsonObject) throws CouldNotPerformException {
        ...
        try {
            ...
            try {
                ...
                HSBColor hsbColor = AWTColorToHSBColorTransformer.transform(javaColor);
                final ColorState.Builder colorState = ColorState.newBuilder();
                colorState.getColorBuilder().setType(Type.HSB).setHsbColor(hsbColor);
                return colorState.build();
            } catch (ClassCastException | IllegalStateException ex) {
                // thrown if value behind COLOR_SPECTRUM_KEY is not an int
                throw new CouldNotPerformException("Could not map from jsonObject[" + color.toString() + "] to [" + ColorState.class.getSimpleName() + "]. Attribute[" + COLOR_SPECTRUM_KEY + "] is not an int");
            }
        } catch (ClassCastException | IllegalStateException ex) {
            // thrown if it is not a json object
            throw new CouldNotPerformException("Could not map from jsonObject[" + jsonObject.toString() + "] to [" + ColorState.class.getSimpleName() + "]. Attribute[" + COLOR_KEY + "] is not an object");
        }
    }
