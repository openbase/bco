 ...
 */
public class CachedUserActivityRegistryRemote {

    private static final Logger LOGGER = LoggerFactory.getLogger(CachedUserActivityRegistryRemote.class);

    private static final SyncObject REMOTE_LOCK = new SyncObject("CachedUserActivityRegistryRemoteLock");

    private static UserActivityRegistryRemote registryRemote;
    ...
    public synchronized static void reinitialize() throws InterruptedException, CouldNotPerformException {
        try {
            ...
            throw new CouldNotPerformException("Could not reinitialize " + CachedUserActivityRegistryRemote.class.getSimpleName() + "!", ex);
        }
    }
        ...
        synchronized (REMOTE_LOCK) {
            if (registryRemote != null) {
                try {
                    ...
                    ExceptionPrinter.printHistory(new FatalImplementationErrorException("Internal remote was locked by an external instance!", CachedUserActivityRegistryRemote.class, ex), LOGGER);
                }
                registryRemote.shutdown();
                registryRemote = null;
            }
