    ...
    public ServiceItemEntry(final DeviceClass deviceClass, final MetaConfig unitHostMetaConfig, final UnitConfig unitConfig, final ServiceConfig serviceConfig) throws InstantiationException {
        ...
        try {
            ...
            try {
                ...
                ExceptionPrinter.printHistory(new CouldNotPerformException("Could not load service template meta config for Service[" + serviceConfig.getServiceDescription().getServiceType().name() + "] of Unit[" + LabelProcessor.getBestMatch(unitConfig.getLabel()) + "]", ex), logger, LogLevel.ERROR);
            }
