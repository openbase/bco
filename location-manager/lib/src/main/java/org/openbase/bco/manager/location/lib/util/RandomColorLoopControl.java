...
public class RandomColorLoopControl {
    ...
    public RandomColorLoopControl(final String locationId, final Collection<HSBColor> colors, final long delay) throws InstantiationException, InterruptedException {
        try {
            this.delay = delay;
            this.colorableLightRemoteList = new ArrayList<>();
            for (UnitConfig unitConfig : Registries.getUnitRegistry().getUnitConfigsByLocation(UnitType.COLORABLE_LIGHT, locationId)) {
